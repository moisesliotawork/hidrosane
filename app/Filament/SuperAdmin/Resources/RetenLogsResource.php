...
use App\Filament\SuperAdmin\Resources\RetenLogsResource\Pages;
use App\Models\Note;
...
class RetenLogsResource extends Resource
{
    protected static ?string $model = Note::class;

    protected static ?string $navigationLabel = 'RETÉN LOGS';
    protected static ?string $navigationGroup = 'Registros';
    protected static ?string $slug = 'reten-logs';
            ...
            ->filters([
                Filter::make('fecha_exacta')
                    ...
                    ->indicateUsing(fn (array $data): ?string =>
                        $data['fecha']
                            ? 'Asignación: ' . \Carbon\Carbon::parse($data['fecha'])->format('d/m/Y')
                            : null
                    ),
            ])
    ...
    {
        return [
            'index' => Pages\ListRetenLogs::route('/'),
        ];
    }
